...
public class practicedashboardpage extends WebDriverBaseTestPage<WebDriverTestPage> {
	...
	public void dashboardPage() throws InterruptedException {
		...
		Validator.verifyTrue(CommonStep.verifyPresent("dashboard.welcome.text"), "You are not on Dashboard page",
	...
	public void verifyQuarterButton() {
		Validator.verifyTrue(CommonStep.verifyPresent("dashboard.quarterly.button"),
				"Quarterly button verification failed", "Quarterly button verified successfully");
	}

	@QAFTestStep(description = "Verify Monthly Button")
	public void verifyMonthlyButton() {
		Validator.verifyTrue(CommonStep.verifyPresent("dashboard.monthly.button"), "Monthly button verification failed",
				"Monthly button verified successfully");
	}
	...
	public void selectQuarter(String sQuarterName) throws InterruptedException {
		...
		do {
			try {
				...
				// QAFWebElement ele1 =
				// driver.findElement(By.xpath(String.format("//div[@class='kn-navigator
				// text-center']//h6[contains(.,'%s')]", sQuarterName)));
			}
